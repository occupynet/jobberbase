			...
			<div id="job-listings">
				...
				{$translations.category.display_only}&nbsp; 
					{section name=tmp loop=$types}
						...
						<a href="{$BASE_URL}{$URL_JOBS_IN_CITY}/{$city_ascii_name}/{$types[tmp].var_name}/" title="{$current_category} {$types[tmp].name}"><span class="{$types[tmp].var_name} job-type">{$types[tmp].name}</span></a>
					{/section}
				</div><!-- #sort-by-type -->
				{/if}
				<h2>
